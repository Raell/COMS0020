...
vector <Rect> detectAndDisplay(Mat frame, vector <Rect> ground_truths, bool merge) {
    ...
    for (int i = 0; i < boxes.size(); i++) {
        rectangle(frame, Point(boxes[i].x, boxes[i].y),
                  Point(boxes[i].x + boxes[i].width, boxes[i].y + boxes[i].height), Scalar(0, 255, 0), 2);
    }
...
float f1_test(vector <Rect> &detected, vector <Rect> &ground_truth, string output, string img_name, float threshold) {
    ...
    for (int i = 0; i < detected.size(); i++) {
        ...
        for (int j = 0; j < ground_truth.size(); j++) {
            ...
            if (matched.find(j) != match.end()) {
                //no point checking for a match with jth ground truth as it has already been matched.
