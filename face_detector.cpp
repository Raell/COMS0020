...
float f1_test(vector<Rect> &detected, vector<Rect> &ground_truth,
              float threshold) {
  int truePositives = 0;
    ...
    for (int j = 0; j < ground_truth.size(); j++) {
      ...
      if (intersectionArea > 0) {
        float matchPercentage =
            (intersectionArea / ground_truth[j].area()) * 100;
  ...
  float precision =
      (float)truePositives / ((float)truePositives + (float)falsePositives);
...
vector<Rect> detectAndDisplay(Mat frame) {
  ...
  cascade.detectMultiScale(frame_gray, detected, 1.1, 1,
  ...
  for (int i = 0; i < detected.size(); i++) {
    rectangle(frame, Point(detected[i].x, detected[i].y),
              Point(detected[i].x + detected[i].width,
                    detected[i].y + detected[i].height),
              Scalar(0, 255, 0), 2);
  }
  return detected;
}
